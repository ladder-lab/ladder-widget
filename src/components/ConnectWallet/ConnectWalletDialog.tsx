...
const StyledMainButton = styled(Button)`
  border-radius: ${({ theme }) => theme.shape.borderRadius * 0.75}px;
  grid-column: 1 / 3;
  height: 100%;
  padding: 10px;
`
...
function WalletConnectButton({
  ...
  return (
    <StyledMainButton onClick={onClick} variant="outlined">
      <StyledMainButtonRow>
        <ButtonContents
          logoSrc={logoSrc}
...
function MetaMaskButton({ walletName, logoSrc, onClick }: ButtonProps) {
  return (
    <StyledSmallButton onClick={onClick} variant="outlined">
...
function NoWalletButton() {
  return (
    <StyledSmallButton onClick={() => window.open(NO_WALLET_HELP_CENTER_URL)} variant="outlined">
      <StyledNoWalletText>
