...
export function useHasPendingApproval(tokenAddress: string | undefined, spender: string | undefined): boolean {
  const { chainId } = useWeb3React()
  const allTransactions = useAtomValue(transactionsAtom)[chainId ?? '']
    ...
    () =>
      ...
      Object.keys(allTransactions ?? {}).some((hash) => {
        ...
        if (tx.receipt || tx.info.type !== TransactionType.APPROVAL) {
          ...
          const approval = tx.info
          if (!approval) return false
          return (
            approval.spenderAddress === spender && approval.tokenAddress === tokenAddress && isTransactionRecent(tx)
          )
        }
      }),
